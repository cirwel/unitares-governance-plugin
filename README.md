...
`start_session` omit the repeated canonical payload; the trimmed lifecycle
responses are marked `response_shape: "routine"`. Read a new identity's uuid
from `agent_uuid`. Other responses may keep the payload under
